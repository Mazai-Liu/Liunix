...
static u32 memory_map_pages; // 映射页占用的页数
...
void mapping_init(){
    ...
    for(idx_t didx = 0; didx < sizeof(KERNEL_PAGE_TABLE) / 4; didx++) {
        ...
        page_entry_t *dir_entry = &pde[didx];
        entry_init(dir_entry, IDX((u32)pte));
        ...
        for(idx_t tidx = 0; tidx < 1024; tidx++, index++) {
            ...
            page_entry_t *table_entry = &pte[tidx];
            entry_init(table_entry, index);
...
void memory_init(u32 magic, u32 addr)
{
    ...
    // 如果是 liunix loader 进入的内核
    if (magic == LIUNIX_MAGIC)
    {
        count = *(u32 *)addr;
